...
export class EmailComponent {
  ...
  extrairDadosDoTexto(texto: string): InterfaceEmailPadrao {
    ...
    const regexConsultor =
      /Consultor\s+([A-Za-z\s\-]+(?:\s+[A-Za-z\s\-]+)*)\s+-\s+[A-Za-z\s\-]+/;

    // Regex para capturar o tipo de cliente
    const regexTipoCliente =
      /Cliente\s+Novo\s*(\d+)\s*Aditivo\s*(\d+)\s*Portabilidades\s*\d*\s*Renegociação\s*(\d+)/;

    // Regex para capturar o nome do cliente (após a segunda ocorrência de "Cliente")
    const regexNomeCliente =
      /(?:Cliente\s+){2}([A-Za-z0-9\s\-\.]+(?:\s+[A-Za-z0-9\s\-\.]+)*)\s*(?=CNPJ)/;

    // Regex para capturar o CNPJ
    const regexCnpj = /CNPJ\s+(\d{14})/;

    // Regex para capturar o "Valor Pedido"
    const regexValorPedido =
      /Valor Pedido\s*R\$\s*([\d\.,]+)\s*Serviços Adicionais/;
